...
> 本仓库 Fork 自 macrozheng/mall（83k+ Stars），
...
mall 是一套电商全栈项目，基于 Spring Boot + MyBatis 实现，采用 Docker 容器化部署。
...
| 接口 | 说明 | 示例 |
| GET /dateCalc/workdays | 计算工作日天数 | 物流发货周期估算 |
| GET /dateCalc/promotion-remaining | 计算促销剩余时间 | 倒计时展示 |
| GET /dateCalc/marketing-calendar | 获取当前营销日历 | 季度/月份信息 |

代码位置: mall-demo/.../DateCalculatorController.java
配套测试: mall-demo/.../DateCalculatorServiceImplTest.java

### 2. SystemDiagnosisController (ruoyi-vue-pro)

JVM 运行时监控接口，实时查看堆内存、线程状态、系统负载等信息。

代码位置: yudao-module-system/.../SystemDiagnosisController.java
